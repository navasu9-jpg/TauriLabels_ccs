...
mkdir -p "tauri-labels.AppDir"
cp -r ../appimage_deb/data/usr "tauri-labels.AppDir"

cd "tauri-labels.AppDir"
mkdir -p "usr/bin"
mkdir -p "usr/lib"
...
( cd "/home/codespace/.cache/tauri" && ( wget -q -4 -N https://github.com/AppImage/AppImageKit/releases/download/continuous/AppRun-${ARCH} || wget -q -4 -N https://github.com/AppImage/AppImageKit/releases/download/12/AppRun-${ARCH} ) )
chmod +x "/home/codespace/.cache/tauri/AppRun-${ARCH}"

# We need AppRun to be installed as tauri-labels.AppDir/AppRun.
# Otherwise the linuxdeploy scripts will default to symlinking our main bin instead and will crash on trying to launch.
cp "/home/codespace/.cache/tauri/AppRun-${ARCH}" AppRun

cp "usr/share/icons/hicolor/512x512/apps/tauri-labels.png" .DirIcon
ln -sf "usr/share/icons/hicolor/512x512/apps/tauri-labels.png" "tauri-labels.png"

ln -sf "usr/share/applications/tauri-labels.desktop" "tauri-labels.desktop"
...
( cd "/home/codespace/.cache/tauri" && wget -q -4 -N https://raw.githubusercontent.com/tauri-apps/linuxdeploy-plugin-gtk/master/linuxdeploy-plugin-gtk.sh )
( cd "/home/codespace/.cache/tauri" && wget -q -4 -N https://github.com/tauri-apps/binary-releases/releases/download/linuxdeploy/linuxdeploy-${linuxdeploy_arch}.AppImage )

chmod +x "/home/codespace/.cache/tauri/linuxdeploy-plugin-gtk.sh"
chmod +x "/home/codespace/.cache/tauri/linuxdeploy-${linuxdeploy_arch}.AppImage"

dd if=/dev/zero bs=1 count=3 seek=8 conv=notrunc of="/home/codespace/.cache/tauri/linuxdeploy-${linuxdeploy_arch}.AppImage"

OUTPUT="tauri-labels_0.1.0_amd64.AppImage" "/home/codespace/.cache/tauri/linuxdeploy-${linuxdeploy_arch}.AppImage" --appimage-extract-and-run --appdir "tauri-labels.AppDir" --plugin gtk ${gst_plugin} --output appimage
